...
d_output1 = c(t=min(t),InitState)
for ( i in seq(min(t)+(t[2]-t[1]),max(t),t[2]-t[1]) )
	{
	if ( is.null(dim(d_output1))==TRUE )
		{
		x1 = d_output1[2]
		x2 = d_output1[3]
		} else 
	...
	d_output1 = rbind(d_output1,c(t=i,x1=Xs[[1]][1],x2=Xs[[1]][2]))
	}

# setwd('C:\\Users\\doliv\\OneDrive\\2_2019_America\\2021\\20211105_Eberhard_discrete_BST\\pictures')
# tiff("Fig_saturated.tiff", height = 20, width = 20, units = 'cm',compression = "lzw", res = 300)
...
# setwd('C:\\Users\\doliv\\OneDrive\\2_2019_America\\2021\\20211105_Eberhard_discrete_BST\\pictures')
# tiff("Fig_saturated_rand1.tiff", height = 20, width = 20, units = 'cm',compression = "lzw", res = 300)
...
lines(out1[,1],out1[,3],col = "darkgrey",lwd=2)
legend(15,.5,legend=c(expression('X'[1]),expression('X'[2]),'ODEs'),pch=c(20,20,NA),pt.cex = c(2,2,NA),lty=c(NA,NA,1),lwd=c(NA,NA,2),col=c('blue','darkcyan','darkgrey'),cex=1.5,bty="n")
# dev.off()
